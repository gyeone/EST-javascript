...
function loadMemoStorage() {
    ...
    const parseMemoData = JSON.parse(localStorage.getItem(MEMO_KEY)) ?? "";
    memoData.push(...parseMemoData);
    // 화면에 메모 목록 출력
    renderMemo();
}
...
function renderMemo() {
    ...
    // 메모가 있으면 반복문으로 각 메모를 화면에 출력s
    memoData.forEach((memo, i) => {
        memoList.innerHTML += `
        <li>${memo.title}:${memo.content}
            <button type="button" id="${i}">삭제</button> 
        </li> `;
    });

    // 삭제 버튼에 이벤트 리스너 추가
    const deleteBtn = document.querySelectorAll("li > button");
...
memoForm.addEventListener("submit", (e) => {
    ...
    memoData.unshift({ title: titleValue, content: contentValue });
    // 로컬스토리지 저장
    saveMemoStorage();
    // 입력 필드 초기화
    memoForm.reset();
